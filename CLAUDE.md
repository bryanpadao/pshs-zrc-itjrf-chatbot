...
- **`processChat(params)`** — called from `Index.html` via `google.script.run`; accepts `{ message, sessionId, userIdentity?, quickStart? }`; validates `sessionId` against UUID v4 format (`/^[0-9a-f-]{36}$/i`) — rejects invalid IDs immediately; rejection logs the bad value to console and returns the message: 'Your session has expired. Please click "Start a New Conversation" to continue.' (Root cause of prior "Invalid session" errors: `sessionId` was initialized as `''` on page load — fixed by `generateSessionId()` in Index.html.); sanitizes input; loads/saves session via CacheService; enforces per-session message limit; pre-fills identity fields from `userIdentity` on first call if provided; if `quickStart` key is set (e.g. `'publication'`, `'cctv'`, `'technical'`), routes directly to `handleQuickStart()` skipping Gemini
...
- **`hasEnoughContext(message)`** — lightweight Gemini call; returns bool — true if message has enough detail to file a ticket; answers YES if the message contains ANY of: an application/device name, a specific symptom or behavior, a network/connectivity issue, or a Filipino/Bisaya IT phrase; answers NO only for completely off-topic content, pure gibberish, or a bare single word with no context; false increments `session.strikeCount` (in chat flow); fails open (returns true) on API error
...
- **Rejection email (inline in `handleApproval()`)** — HTML rejection notification is built inline inside `handleApproval()` (no separate helper function). Sent to col R (requester email) when status → `Rejected`; reuses the navy-header template from `sendApprovalEmail()`; subject and body indicate whether rejection was at the supervisor or director stage
...
- **`generateKbEntry(jrfNo)`** — generates a KnowledgeBase entry from a Completed ticket using Gemini; reads col F (problem), col L (action taken), and col G (recommendation type); makes a single-turn Gemini call and returns `{ issue, solution, category, keywords, jrfNo }` for IT staff review before saving; returns `{ error: true, message }` if ticket not found, not Completed, or fields are empty; calls `_requireDashboardAuth()`
- **`saveKbEntry(issue, solution, category, keywords)`** — appends a reviewed KnowledgeBase entry to the KnowledgeBase sheet (cols A–D: Issue / Solution / Category / Keywords); validates that issue and solution are non-empty; calls `_requireDashboardAuth()`
...
- `sessionId` generated by `generateSessionId()` on page load and on `resetChat()`. Uses `crypto.randomUUID()` with a `Math.random()` hex fallback — always produces lowercase UUID v4 (`xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx`). Regenerated when `resetChat()` is called, or when identity resolves successfully after a prior error state (e.g. user switched from wrong account to correct one). Sent on every `processChat()` call via `google.script.run`.
...
- **"+ KB" button on Completed tickets**: Opens a KB modal that calls `generateKbEntry()` to auto-generate a KnowledgeBase entry using Gemini from the ticket's problem (col F) and action taken (col L); IT staff reviews and edits the four fields (Issue, Solution, Category, Keywords) before saving; submits via `saveKbEntry()`; modal stays open until server call completes; success shows `showToast()` confirmation
...
| `sessionId` was `''` on page load — every first message rejected with "Invalid session" | Fixed: `generateSessionId()` produces UUID v4 using `crypto.randomUUID()` + Math.random fallback. `resetChat()` and the identity error-recovery path both call `generateSessionId()`. Root causes: A (sessionId not initialized) + B (no UUID generator). |
| `sessionId = response.sessionId \|\| sessionId` at Index.html line 1179 | Dead code — server never returns `sessionId` in processChat() responses. Safe to remove in next cleanup pass. |
| "Microsoft Word is laggy" falsely rejected by hasEnoughContext() | Fixed: hasEnoughContext() Gemini prompt updated to disjunctive — YES on ANY of: app name, symptom, network issue, Bisaya phrase; NO only for off-topic/gibberish/bare single word |
| paraphraseDescription() offtopic filter applied to chat flow | Fixed: offtopic gate is now conditional on `requestType`; chat flow passes no requestType → gate skipped entirely; form panel passes requestType → gate runs |
| No visibility into chat rejection reasons | Fixed: handleChat() logs `console.error()` before each isGibberish() and hasEnoughContext() strike increment — rejection reason and first 50 chars of input visible in Apps Script execution log |
